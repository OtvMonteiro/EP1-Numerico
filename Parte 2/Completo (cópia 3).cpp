...
// ---- Determinacao de uk(T,xi) por Cranck Nicolson
	...
	for(int i=0; i<Nf-1; i++){
		...
		//Extra, visualizacao da matriz coluna//cout<<bmmq[i]<<endl;
	}
  ...
  //Solucionando DLt*x=y
   for(int i=Nf-1; i>-1; i--){
	...
	double E=0.0;	//erro quadratico
	double E_ruido=0.0;//erro quadratico calculado com uT tratado com ruido aleatorio
	//Calculo de E
	for(int i=1; i!=N; i++){
		for(int k=1;k<nf+1;k++){uT_encontrado[i] += a[k]*u[i][k];}
		E += pow(uT[i] - uT_encontrado[i]  ,2);//Calculando o quadrado da diferenca e somando
		E_ruido += pow(uT_ruido[i] - uT_encontrado[i]  ,2);//O mesmo, mas para dados com ruido
	}
	//Como calculamos a integral de 0 a 1 do quadrado das diferencas, precisamos tirar a raiz quadrada
	E = sqrt(E);        E_ruido = sqrt(E_ruido);
	...
	if(item=='c'||item=='d'){		
		...
		if(gui_choice1!='n'){
			for(int i=0; i!=N+1; i++){//Alternativamente posso usar uT_encontrado, ja calculado de 1 a N-1
				double aux=0.0;
